...
async function checkLabelInName(page: Page): Promise<CheckResult> {
  const issues = await page.evaluate(() => {
    ...
    elements.forEach((el) => {
      ...
      const visibleText = (htmlEl.innerText || htmlEl.textContent || "").trim().toLowerCase()
...
async function checkNonTextContrast(page: Page): Promise<CheckResult> {
  ...
  return {
    ...
    criterion: "1.4.11",
    name: "非テキストコントラスト",
    result: issues.length === 0 ? "pass" : issues.length <= 3 ? "warning" : "fail",
    details:
      issues.length === 0
...
async function checkHeadingStructure(page: Page): Promise<CheckResult> {
  ...
  return {
    ...
    name: "見出し構造",
    result:
      issues.problems.length === 0 ? "pass" : issues.problems.length <= 2 ? "warning" : "fail",
    details:
...
async function checkAriaLive(page: Page): Promise<CheckResult> {
  const result = await page.evaluate(() => {
    ...
    const dynamicElements = document.querySelectorAll(dynamicSelectors.join(","))
  ...
  return {
    ...
    result: result.missingLive.length > 0 ? "warning" : result.found.length > 0 ? "pass" : "pass",
    details:
...
async function checkAutoplayMedia(page: Page): Promise<CheckResult> {
  const autoplayElements = await page.evaluate(() => {
    const elements = document.querySelectorAll("audio[autoplay], video[autoplay]")
...
async function checkCharKeyShortcuts(page: Page): Promise<CheckResult> {
  ...
  return {
    ...
    criterion: "2.1.4",
    name: "文字キーのショートカット",
    result: allIssues.length === 0 ? "pass" : issues.length > 0 ? "warning" : "warning",
    details:
      allIssues.length === 0
        ? "文字キーショートカットは検出されず"
